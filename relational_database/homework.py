...
def task_8_count_customers_by_city(cur):
    ...
    cur.execute(
        """
        SELECT COUNT(CustomerName) count, city
        FROM customers
        GROUP BY city;
        """
    )
...
def task_12_list_suppliers_from_specified_countries(cur):
    ...
    cur.execute("""
        ...
        FROM suppliers
        WHERE UPPER(country) IN ('USA', 'UK', 'JAPAN');
    """)
    return cur.fetchall()
...
def task_16_match_all_customers_and_suppliers_by_country(cur):
    ...
    """
    cur.execute("""
        SELECT customername, c.address, c.country customercountry, s.country suppliercountry, suppliername
        FROM customers c
